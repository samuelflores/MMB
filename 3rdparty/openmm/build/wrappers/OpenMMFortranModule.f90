...
MODULE OpenMM_Types
    ...
    type OpenMM_BAOABLangevinIntegrator
        integer*8 :: handle = 0
    end type
...
stepSize)
        ...
        subroutine OpenMM_BAOABLangevinIntegrator_create(result, temperature, &
frictionCoeff, &
stepSize)
            use OpenMM_Types; implicit none
            type (OpenMM_BAOABLangevinIntegrator) result
            real*8 temperature
            real*8 frictionCoeff
            real*8 stepSize
        end subroutine
        subroutine OpenMM_BAOABLangevinIntegrator_destroy(destroy)
            use OpenMM_Types; implicit none
            type (OpenMM_BAOABLangevinIntegrator) destroy
        end subroutine
        function OpenMM_BAOABLangevinIntegrator_getTemperature(target)
            use OpenMM_Types; implicit none
            type (OpenMM_BAOABLangevinIntegrator) target
            real*8 OpenMM_BAOABLangevinIntegrator_getTemperature
        end function
        subroutine OpenMM_BAOABLangevinIntegrator_setTemperature(target, temp)
            use OpenMM_Types; implicit none
            type (OpenMM_BAOABLangevinIntegrator) target
            real*8 temp
        end subroutine
        function OpenMM_BAOABLangevinIntegrator_getFriction(target)
            use OpenMM_Types; implicit none
            type (OpenMM_BAOABLangevinIntegrator) target
            real*8 OpenMM_BAOABLangevinIntegrator_getFriction
        end function
        subroutine OpenMM_BAOABLangevinIntegrator_setFriction(target, coeff)
            use OpenMM_Types; implicit none
            type (OpenMM_BAOABLangevinIntegrator) target
            real*8 coeff
        end subroutine
        function OpenMM_BAOABLangevinIntegrator_getRandomNumberSeed(target)
            use OpenMM_Types; implicit none
            type (OpenMM_BAOABLangevinIntegrator) target
            integer*4 OpenMM_BAOABLangevinIntegrator_getRandomNumberSeed
        end function
        subroutine OpenMM_BAOABLangevinIntegrator_setRandomNumberSeed(target, seed)
            use OpenMM_Types; implicit none
            type (OpenMM_BAOABLangevinIntegrator) target
            integer*4 seed
        end subroutine
        subroutine OpenMM_BAOABLangevinIntegrator_step(target, steps)
            use OpenMM_Types; implicit none
            type (OpenMM_BAOABLangevinIntegrator) target
            integer*4 steps
        end subroutine
