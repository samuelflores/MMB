...
//		cout << "torsion " << r << " is real torsion" << endl;
         ...
         //   myResidueNumber.ResidueNumber += stoi(myNTC_PAR_BondRow.atom_shift[0]);
                ...
                else if(stoi(myNTC_PAR_BondRow.atom_shift[0]) == 1) myResidueNumber = residueNumber2;
                ...
                Vec3 state_1 = myBiopolymerClassContainer.calcAtomLocationInGroundFrame(state,chainId1,myResidueNumber,myNTC_PAR_BondRow.residue1Atom[0]);

                if(stoi(myNTC_PAR_BondRow.atom_shift[1]) == 0) myResidueNumber = residueNumber1;
                else if(stoi(myNTC_PAR_BondRow.atom_shift[1]) == 1) myResidueNumber = residueNumber2;
                ...
                Vec3 state_2 = myBiopolymerClassContainer.calcAtomLocationInGroundFrame(state,chainId1,myResidueNumber,myNTC_PAR_BondRow.residue1Atom[1]);

                if(stoi(myNTC_PAR_BondRow.atom_shift[2]) == 0) myResidueNumber = residueNumber1;
                else if(stoi(myNTC_PAR_BondRow.atom_shift[2]) == 1) myResidueNumber = residueNumber2;
                ...
                Vec3 state_3 = myBiopolymerClassContainer.calcAtomLocationInGroundFrame(state,chainId1,myResidueNumber,myNTC_PAR_BondRow.residue1Atom[2]);

                if(stoi(myNTC_PAR_BondRow.atom_shift[3]) == 0) myResidueNumber = residueNumber1;
                else if(stoi(myNTC_PAR_BondRow.atom_shift[3]) == 1) myResidueNumber = residueNumber2;
                ...
                Vec3 state_4 = myBiopolymerClassContainer.calcAtomLocationInGroundFrame(state,chainId1,myResidueNumber,myNTC_PAR_BondRow.residue1Atom[3]);

                torqueConstant = myNTC_PAR_BondRow.torqueConstant;

                Vec3 d_d1 = state_2 - state_1;
                Vec3 d_d2 = state_3 - state_2;
                Vec3 d_d3 = state_4 - state_3;
    ...
    // pot_angle = torqueConstant*(dist_ang/57.295779513);//(exp(-(pow(dist_ang,2)/(2.0*pow(l_param,2)))))*(dist_ang/57.295779513)/(2.0*pow(l_param/57.295779513,2))*(sin(dist_ang/57.295779513));
                ...
                if((myParameterReader.ntc_class_container.getNTC_Class(r)).meta == 0) {
                    pot_angle = torqueConstant*(myParameterReader.ntc_class_container.getNTC_Class(r)).weight*(-sin((dist_ang + 180.0)/57.295779513))*(360.0/57.295779513 + 1.0)/(1.0 + myNTC_PAR_BondRow.CONFALVALUE)/(360.0/57.295779513);
       //{ < - MIND THIS BRACE
      ...
      //   cout << pot_angle << "  -- pot_angle " << endl;
                ...
                } else if((myParameterReader.ntc_class_container.getNTC_Class(r)).meta == 1) {
                    dih = 0;
                    value = -1;
                    bias = 0;
                    ...
                    if(isfinite(angle) == 1) {
                        ...
                        if(myBiopolymerClassContainer.counter[(myParameterReader.ntc_class_container.getNTC_Class(r)).count] > 0.0)
                            myBiopolymerClassContainer.prob[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][i] = myBiopolymerClassContainer.hist[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][i]/myBiopolymerClassContainer.counter[(myParameterReader.ntc_class_container.getNTC_Class(r)).count];

                        if(value > 0 &&
                           myBiopolymerClassContainer.prob[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][value] > 1e-3 &&
                           myBiopolymerClassContainer.prob[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][value+1] > 1e-3 &&
                           value < 360)
                            bias = 2.479*(log(myBiopolymerClassContainer.prob[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][value]/(1e-3))-log(myBiopolymerClassContainer.prob[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][value+1]/(1e-3)));
                        if(prob[value] > 1e-3 &&
                           myBiopolymerClassContainer.prob[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][value+1] > 1e-3 &&
                           value == 360)
                            bias = 2.479*(log(myBiopolymerClassContainer.prob[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][value]/(1e-3))-log(myBiopolymerClassContainer.prob[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][1]/(1e-3)));
       ...
       /* if( isfinite(log(prob[value]/(1e-3))) == 1 && isfinite(log(prob[value+1]/(1e-3))) == 1) {
         ...
         cout << log(prob[value]/(1e-3)) << log(prob[value+1]/(1e-3)) << " log prob " << endl;
                        ...
                        Vec3 torque = d_d2/d_d2.norm()*(pot_angle)/(1.0+(myParameterReader.ntc_class_container.getNTC_Class(r)).weight2)*(myParameterReader.ntc_class_container.getNTC_Class(r)).weight;
        ...
        // cout << pot_angle << " pot angle " << endl;
                        ...
                        if(isfinite(log(myBiopolymerClassContainer.prob[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][value]/(1e-3))) == 1 &&
                           isfinite(log(myBiopolymerClassContainer.prob[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][value+1]/(1e-3))) == 1) {
                ...
                Vec3 state_1 = myBiopolymerClassContainer.calcAtomLocationInGroundFrame(state,chainId1,residueNumber1,myNTC_PAR_BondRow.residue1Atom[0]);
                ...
                Vec3 state_2 = myBiopolymerClassContainer.calcAtomLocationInGroundFrame(state,chainId1,residueNumber2,myNTC_PAR_BondRow.residue1Atom[1]);
                ...
                if((myParameterReader.ntc_class_container.getNTC_Class(r)).meta == 0) {
                    ...
                    if(d < 3.0) {
                        ...
                        if(myBiopolymerClassContainer.counter_d[(myParameterReader.ntc_class_container.getNTC_Class(r)).count] > 0.0)
                            myBiopolymerClassContainer.prob_d[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][i] = myBiopolymerClassContainer.hist_d[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][i]/myBiopolymerClassContainer.counter_d[(myParameterReader.ntc_class_container.getNTC_Class(r)).count];
                        else if(value > 0 &&
                            myBiopolymerClassContainer.prob_d[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][value] > 1e-3 &&
                            myBiopolymerClassContainer.prob_d[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][value+1] > 1e-3 &&
                            value < 31)
                            bias = 2.479*(log(myBiopolymerClassContainer.prob_d[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][value]/(1e-3))-log(myBiopolymerClassContainer.prob_d[(myParameterReader.ntc_class_container.getNTC_Class(r)).count][value+1]/(1e-3)));
...
Real NTC_Torque::return_angle(Vec3 cross_1,Vec3 cross_2,Vec3 cross_3,Vec3 d_d2) const
{
    ...
    else if(scalar_product < -1.0) scalar_product = -1.0;
