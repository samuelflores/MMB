...
void ParameterReader::applyAtomSprings(SimbodyMatterSubsystem & matter, GeneralForceSubsystem & forces)

{
    cout << __FILE__ <<":"<<__LINE__<<" : Convert Threading instructions to springs."<<endl;
...
void ParameterReader::updateAllResiduesWithin(int index, String chainID, int resID, double radius){
    ...
    includeAllResiduesWithinVector[index].chain = chainID;
    includeAllResiduesWithinVector[index].residue = res;
    includeAllResiduesWithinVector[index].radius = radius;
}
                                                            ...
                                                            int resID)
{
    ...
    if(index < 0 || index >= includeAllNonBondAtomsInResidueVector.size()){
        ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": you tried to update a non existing includeAllNonBondAtomsInResidue command." << endl;
        ErrorManager::instance.treatError();
    }

    includeAllNonBondAtomsInResidueVector[index].chain = chainID;
    includeAllNonBondAtomsInResidueVector[index].residue = res;
}
...
{
    ParameterStringClass parameterStringClass;
    vector<String> mystring;
    stringstream u;
    const int numFields = 10 ;
    cout << paramstr << endl;
    mystring.clear();
    for (int w = 0;w<numFields;w++) mystring.push_back("");
    u.str(paramstr); 
    u>>mystring[0]>>mystring[1]>>mystring[2]>>mystring[3]>>mystring[4]>>mystring[5]>>mystring[6]>>mystring[7]>>mystring[8]>>mystring[9]; 
    parameterStringClass.clear();
    for (int j = 0; j < numFields; j++){
        parameterStringClass.add(mystring[j]); 
    }
                                                 ...
                                                 const bool readExcept)
{
    ...
    if  (((parameterStringClass.getString(0)).compare(  "insertResidue") == 0))  {
        ...
        String myChain = parameterStringClass.getString(1);
            ...
            if (safeParameters) if (!mobilizerContainer.isEmpty()) {
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<": You have previously called some sort of mobilizer command. Please move all such commands, which reference residue numbers, below the deleteResidues command. Otherwise there  too much potential to reference a deleted residue."<<endl;
                ErrorManager::instance.treatError();
            }
    ...
    if ( ((parameterStringClass.getString(0)).compare("atomSpring")      == 0) ||
                ...
                ((parameterStringClass.getString(0)).compare("atomTether")      == 0))
        {
            ...
            if (myBiopolymerClassContainer.hasChainID(myAtomSpring.atom1Chain)) {
                ...
                ErrorManager::instance <<__FILE__<<":"<<__LINE__<<"Error! No biopolymer or monoAtoms container has this chain ID!"<<endl;
                ErrorManager::instance.treatError();
            }
    ...
    if ( ((parameterStringClass.getString(0)).compare("nucleicAcidDuplex")      == 0) ) 
    {
        ...
        // format:  nucleicAcidDuplex <Chain ID A> <start residue A> <end residue A> <Chain B ID> <start residue B> <end residue B> 
        // where start residue A will be Watson-Crick base paired with  end residue B, etc.  we require: (start residue A) <= (end residue A) and (start residue B) >= (end residue B), because duplexes are antiparallel.
    ...
    if ((parameterStringClass.getString(0)).compare("gappedThreading")      == 0) 
    {
        cout<<__FILE__<<":"<<__LINE__<< " You have called the gappedThreading command. This is the same as the \"threading\" command, but instead of requiring you to provide aligned stretches of residues, it figures out the gapped alignment for you. It uses a simple scoring function, which differentiates between \"match\" (residues the same), \"mismatch\" (residues not the same), gap opening, and gap extension."<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" Syntax:  <gappedThreading> <Chain A>  <Chain-B>  [forceConstant]"<<endl;
        double defaultForceConstant = 3.;
        cout<<"Springs will be applied to pull like-named atoms in corresponding residues in <chain A> vs. <chain B> together. forceConstant is optional and defaults to "<<defaultForceConstant <<endl;
        String      chainA = parameterStringClass.getString(1);
        ...
        atomSpringContainer.addGappedThreading(chainA, chainB, myForceConstant, false, myBiopolymerClassContainer);
        return;
    } //End gappedThreading
            ...
            ((parameterStringClass.getString(0)).compare("proteinBackboneThreading")      == 0) ) 
    {
        ...
        cout<<"Format:  <threading | proteinBackboneThreading> <Chain A> <start-residue-A> <end-residue-A> <Chain-B> <start-residue-B> <end-residue-B> [forceConstant]"<<endl;
        cout<<"where start-residue-A will be aligned with start-residue-B, etc.  we require: end-residue > start-residue. forceConstant is optional and defaults to "<<defaultForceConstant <<endl;
        double myForceConstant = 1000000000.0; // set to super high value just to make sure it's being reset later.
        if ((parameterStringClass.getString(7)).length() >0) 
            myForceConstant= myAtoF(userVariables,parameterStringClass.getString(7).c_str());
        else myForceConstant= defaultForceConstant;

        ThreadingStruct thread;
        thread.chainID1 = parameterStringClass.getString(1);
        thread.residueStart1 = myBiopolymerClassContainer.residueID(userVariables,parameterStringClass.getString(2),thread.chainID1);
        thread.residueEnd1 = myBiopolymerClassContainer.residueID(userVariables,parameterStringClass.getString(3),thread.chainID1);
        thread.chainID2 = parameterStringClass.getString(4);
        thread.residueStart2 = myBiopolymerClassContainer.residueID(userVariables,parameterStringClass.getString(5),thread.chainID2);
        thread.residueEnd2 = myBiopolymerClassContainer.residueID(userVariables,parameterStringClass.getString(6),thread.chainID2);
        thread.forceConstant = myForceConstant;
        thread.backboneOnly = false;
        if ((parameterStringClass.getString(0)).compare("proteinBackboneThreading") == 0) 
            thread.backboneOnly = true;

        atomSpringContainer.addThreading(thread, myBiopolymerClassContainer);
    ...
    if ((parameterStringClass.getString(0)).compare("constraint") == 0) {
        cout<<__FILE__<<":"<<__LINE__<<" : Syntax for custom molecules:  constraint <chain> <atom name> Weld Ground "<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" : Syntax for biopolymers:  constraint <chain 1> <residue ID 1> Weld Ground "<<endl;
        ...
        if ( (parameterStringClass.getString(4).length()>0) &&(!(parameterStringClass.getString(5).length() > 0))){ 
        ...
        /*else if ((parameterStringClass.getString(5).length()>0) && (!(parameterStringClass.getString(6).length()>0)))
            if (parameterStringClass.getString(4).compare("Weld") == 0) 
        { // this is teh case of "constraint <chain 1> <residue ID 1> <atom name 1> Weld Ground"
                cout<<__FILE__<<":"<<__LINE__<<" Detected that this is the case constraint <chain 1> <residue ID 1> <atom name 1> Weld Ground"<<endl;
        String myChain1         = parameterStringClass.getString(1);
        String myResidue1String = parameterStringClass.getString(2);
        String myAtomName1      = parameterStringClass.getString(3);
                if (myBiopolymerClassContainer.hasChainID(myChain1) {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : The specified chain "<<myChain1<< " is a biopolymer, not a custom molecule!"<<endl;
            ErrorManager::instance.treatError();
            myBiopolymerClassContainer.addConstraintToGround(userVariables, myChain1, myResidue1String,myAtomName1, constraintToGroundContainer);
                }
                else if (moleculeClassContainer.hasChainID(myChain1) 
            moleculeClassContainer.addConstraintToGround(userVariables, myChain1, myResidue1String,myAtomName1, constraintToGroundContainer);
                else {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : The specified chain "<<myChain1<< " has not been instantiated as a biopolymer or custom molecule"<<endl;
            ErrorManager::instance.treatError();
                }
        }*/
    ...
    if ((parameterStringClass.getString(0)).compare("constrainToGround") == 0) {
        ...
        if ((parameterStringClass.getString(1).length() == 0) ) {
            ...
            if ( myBiopolymerClassContainer.updBiopolymerClass(parameterStringClass.getString(1)).getFirstResidueID() == 
                    myBiopolymerClassContainer.residueID(userVariables, parameterStringClass.getString(2), parameterStringClass.getString(1)) )  //  .updBiopolymerClass(parameterStringClass.getString(1)).residueID(parameterStringClass.getString(2)))
    ...
    if  (((parameterStringClass.getString(0)).compare("applyMobilizersWithin") == 0))  {
        ...
        mobilizerWithin.Chain = parameterStringClass.getString(3);
        mobilizerWithin.Residue = myBiopolymerClassContainer.residueID(userVariables,parameterStringClass.getString(4), mobilizerWithin.Chain);
        mobilizerWithin.Radius =  myAtoF(userVariables,parameterStringClass.getString(2).c_str());
        mobilizerWithin.BondMobilityString = parameterStringClass.getString(1);
    ...
    if  (((parameterStringClass.getString(0)).compare("mobilizeInterfaces") == 0))  {
        ...
        int i = 3;
        String versusString = "Versus";
        while ((parameterStringClass.getString(i).length() >0)  &&
               (parameterStringClass.getString(i).compare(versusString ) != 0 )) { // if parameter i is "versus", stop and start reading partnerChains
            
            chains.push_back( parameterStringClass.getString(i));
            i++;
        }
        if (parameterStringClass.getString(i).compare(versusString) == 0 ) { 
            i++;
        ...
        mobilizerContainer.mobilizerInterfaceContainer.addInterface(chains, partnerChains, depth, bondMobilityString);
    ...
    if  (((parameterStringClass.getString(0)).compare( "setDefaultMDParameters") == 0))  {
        //#setDefaultMDParameters = aToBool(parameterStringClass.getString(0),(parameterStringClass.getString(1)).c_str());     
        parameterStringClass.validateNumFields(1);
        //#if (verbose) cout<<__FILE__<<":"<<__LINE__<<"  setDefaultMDParameters ="<< setDefaultMDParameters<<endl; 
        //if (setDefaultMDParameters) {
        //addNASTForces =0;
    ...
    if ((parameterStringClass.getString(0)).compare("waterDroplet") ==0) {
        cout<<"syntax:  waterDroplet chainID X Y Z radius [tetherStrength [tetherType]]"<<endl;
        cout<<__FILE__<<":"<<__LINE__<<" Note that in MMB 2.10 and earlier, we took the radius in Å.  We are going back to nm for consistency, with apologies for the confusion."<<endl;
    ...
    if ((parameterStringClass.getString(0)).compare("waterDropletAboutResidue") ==0) {
        cout<<"syntax:  waterDropletAboutResidue biopolymerChainID biopolymerResidueNumber radius tetherStrength waterDropletChainID"<<endl;
    ...
    if ((parameterStringClass.getString(0)).compare("loadSequencesFromPdb") ==0) {
        ...
        if(currentStage > firstStage || parameterStringClass.getString(1).length() == 0)
        {
            ...
            loadSequencesFromPdb(previousFrameFileName);
            return;
        }

        if(parameterStringClass.getString(2).length() > 0) 
        {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : loadSequencesFromPdb takes only one optional parameter which is a pdb file name."<<endl;
             ErrorManager::instance.treatError();
        }
        ...
        { 
            ...
            loadSequencesFromPdb(pdbFileName );
        ...
        ((parameterStringClass.getString(0)).compare( "includeResidues") == 0) )  
    {
        ...
        for (ResidueID i = myBiopolymerClassContainer.residueID(userVariables,(parameterStringClass.getString(2)),parameterStringClass.getString(1) )  ; i <=myBiopolymerClassContainer.residueID (userVariables, (parameterStringClass.getString(3)),parameterStringClass.getString(1)); myBiopolymerClassContainer.updBiopolymerClass(parameterStringClass.getString(1)).incrementResidueID( i)  ) {
            ...
            myIncludeAllNonBondAtomsInResidue.chain   = parameterStringClass.getString(1);
            myIncludeAllNonBondAtomsInResidue.residue   =i;  
            myBiopolymerClassContainer.updBiopolymerClass(myIncludeAllNonBondAtomsInResidue.chain ).validateResidueID( myIncludeAllNonBondAtomsInResidue.residue );
            includeAllNonBondAtomsInResidueVector.push_back(myIncludeAllNonBondAtomsInResidue);     
            //if (i == ResidueID ((parameterStringClass.getString(3)))) 
    ...
    if (( ((parameterStringClass.getString(0)).compare( "includeAllResiduesWithin") == 0)) ||
        ...
        }
        {
            ...
            myAllResiduesWithin.radius   = myAtoF(userVariables,parameterStringClass.getString(1).c_str());
            myAllResiduesWithin.chain   = parameterStringClass.getString(2);
            if (myBiopolymerClassContainer.hasChainID(myAllResiduesWithin.chain)) {
            myAllResiduesWithin.residue   = myBiopolymerClassContainer.residueID(userVariables,parameterStringClass.getString(3),myAllResiduesWithin.chain );
        if (parameterStringClass.getString(3).length() == 0) 
        {
            ErrorManager::instance <<__FILE__<<":"<<__LINE__<<" : You have not provided enough parameters for this command."<<endl; ErrorManager::instance.treatError();
        }
            }
            else if (moleculeClassContainer.hasChainID(myAllResiduesWithin.chain)) {
                myAllResiduesWithin.residue   = ResidueID(1,' ');//(userVariables,parameterStringClass.getString(3) );
            }
    ...
    if ((parameterStringClass.getString(0)).compare("leontisWesthofInFileName") ==0) {
        ...
        ErrorManager::instance.treatError();
        parameterStringClass.validateNumFields(2);
        leontisWesthofInFileName =((parameterStringClass.getString(1))); //the const_cast thing turns this const char* into char*
...
void ParameterReader::initializeFromFileOnly(const char * parameterFileName ) {
    ...
    const int numFields = 10 ;
    vector <String> mystring;//[numFields];//1;   
    ParameterStringClass parameterStringClass;
    //for (int w = 0;w<numFields;w++) mystring.push_back("");//[w]="";
    int readStage = 0;
...
void ParameterReader::loadSequencesFromPdb(const char * pdbFileName) { //, vector<std::string> deletedResidueVector ){
    ...
    myBiopolymerClassContainer.loadSequencesFromPdb( pdbFileName, proteinCapping );
...
void ParameterReader::postInitialize(){
    ...
    //densityContainer.stuffDensityStretchVector(myBiopolymerClassContainer); 
    densityContainer.printDensityStretches    (); 
    //moleculeClassContainer.initializeCompounds (dumm);
    if (safeParameters) {
...
void ParameterReader::clearForces(){
    ...
    waterDropletContainer.clear();
    includeAllNonBondAtomsInResidueVector.clear();
...
void ParameterReader::clearConstraints(){
    ...
    mobilizerContainer.mobilizerInterfaceContainer.clear();
...
void ParameterReader::initializeDefaults(const char * leontisWesthofInFileName){
    ...
    this->leontisWesthofInFileName = strdup(leontisWesthofInFileName);
    ...
    electroDensityFileName = "electroensityFileName-NOT-SET";
