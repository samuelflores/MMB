...
void ConstrainedDynamics::initializeBiopolymers(CompoundSystem & system){
    ...
    _parameterReader->myBiopolymerClassContainer.initializeAtomInfoVectors(_matter);
}
...
void ConstrainedDynamics::setInterfaceMobilizers(CompoundSystem & system, SimbodyMatterSubsystem & matter, State & state){
    ...
    //_parameterReader->myBiopolymerClassContainer.initializeAtomInfoVectors( matter);
    //cout<<__FILE__<<":"<<__LINE__<<endl;
    _parameterReader->myBiopolymerClassContainer.validateAtomInfoVectors();
    _parameterReader->mobilizerContainer.addMobilizerStretchesToVector(_parameterReader->myBiopolymerClassContainer);
...
void ConstrainedDynamics::initializeCustomForcesConstraints(){
    // _leontisWesthofClass.initialize((_parameterReader->leontisWesthofInFileName));
    ...
    if (_parameterReader->includeIntraChainInterfaceVector.size() >0) 
    {
       ...
       {
           _parameterReader->myBiopolymerClassContainer.addIntraChainInterfaceResidues(_parameterReader->includeIntraChainInterfaceVector[i].Chain,  _parameterReader->includeAllNonBondAtomsInResidueVector , _parameterReader->includeIntraChainInterfaceVector[i].Depth ,  _matter,_state) ;
       }
    }
    if (_parameterReader->includeIntraChainInterfaceVector.size() >0) {
        _parameterReader->myBiopolymerClassContainer.initializeAtomInfoVectors(_matter,_dumm); // investigate moving this outside the conditional, so it's available for DensityForce below.
    ...
    cout<<__FILE__<<":"<<__LINE__<<" Did you intend to use PhysicsWhereYouWantIt? Checking size of _parameterReader->includeAllNonBondAtomsInResidueVector.size() = "<<_parameterReader->includeAllNonBondAtomsInResidueVector.size()<<endl;
    ...
    (_parameterReader->includeAllNonBondAtomsInResidueVector.size() > 0) ||
    ...
    _parameterReader->myBiopolymerClassContainer.includeAllResiduesWithin(_parameterReader->includeAllResiduesWithinVector, _parameterReader->includeAllNonBondAtomsInResidueVector, _state); 
    ...
    if ((_parameterReader->includeAllNonBondAtomsInResidueVector.size() >  0)) 
    {
        _parameterReader->myBiopolymerClassContainer.includeAllNonBondAtomsInResidues(_parameterReader->includeAllNonBondAtomsInResidueVector, _state, _dumm);
        ...
        _parameterReader->myBiopolymerClassContainer.printAllIncludedResidues ( _parameterReader->includeAllNonBondAtomsInResidueVector);
    }
