...
String  BiopolymerClass::getSubSequence(const ResidueID startResidue, const ResidueID endResidue)
{
    ...
    std::cout<<__FILE__<<":"<<__LINE__<<" You have requested the subsequence of chain "<<getChainID()<<", which is : "<<sequence<<" bounded by "<<startResidue.outString()     <<", "<<endResidue.outString()<<". Result is: "<<subSequence<<std::endl;
    return subSequence;
}
...
void BiopolymerClass::constrainRigidSegmentsToGround(CompoundSystem & system,  SimbodyMatterSubsystem & matter,State & state, ConstraintToGroundContainer & myConstraintToGroundContainer , bool toGround = true, ResidueID baseResidue = ResidueID() ) {
    ...
    while (myResidueID <= getLastResidueID()) {
            ...
            MassProperties myBodyMassProperties = myBody.getBodyMassProperties(state);
                    ...
                    cout<<__FILE__<<":"<<__LINE__<<"got getFirstResidueMobilizerType() : "<< getFirstResidueMobilizerType()               <<endl;
                    cout<<__FILE__<<":"<<__LINE__<<" ((myResidueID == getFirstResidueID()) evaluates to : "<< (myResidueID == getFirstResidueID())<<endl;
                    cout<<__FILE__<<":"<<__LINE__<<" (getFirstResidueMobilizerType().compare(Weld)==0  ) evaluates to : "<<  (getFirstResidueMobilizerType().compare("Weld")==0  )<<endl;
            if (myBodyMassProperties.getMass() > getRepresentativeAtomMassThreshold()){ 
                ...
                */
                } else if (myResidueID <  getLastResidueID()) 
                {
                    ...
                    cout<<__FILE__<<":"<<__LINE__<<"myResidueID ="<<myResidueID.outString()<<endl; 
...
void BiopolymerClass::physicsZone(vector<AllResiduesWithin> & myIncludeAllResiduesWithinVector , double radius, SimbodyMatterSubsystem & matter,State & state) {
    ...
    for (int i = 0; i < atomInfoVector.size() ; i++) {
            ...
            if (myBodyMassProperties.getMass() < 40){
                ...
                cout<<__FILE__<<":"<<__LINE__<<" Implementng physicsRadius. Include around :"<< endl;
...
void BiopolymerClassContainer::printAllIncludedResidues (vector<IncludeAllNonBondAtomsInResidue> & includeAllNonBondAtomsInResidueVector ) {
    ...
    for (int i = 0 ; i < (int)includeAllNonBondAtomsInResidueVector.size(); i++) {
        ...
        cout<<__FILE__<<":"<<__LINE__<<": chain = "<<myIncludeAllNonBondAtomsInResidue.getChain()<<", residue = "<<myIncludeAllNonBondAtomsInResidue.getResidue().outString()<<endl;
    }
}
...
void BiopolymerClassContainer::includeAllResiduesWithin (const vector<AllResiduesWithin> & includeAllResiduesWithinVector, 
    ...
    for ( int j = 0 ; j < neighborList.size(); j++) 
    {
        ...
        //IncludeAllNonBondAtomsInResidue incl1 (concatenatedAtomInfoVector[id1].chain, concatenatedAtomInfoVector[id1].residueID);// = {concatenatedAtomInfoVector[id1].chain, concatenatedAtomInfoVector[id1].residueID};
        IncludeAllNonBondAtomsInResidue incl1; incl1.setChain(concatenatedAtomInfoVector[id1].chain); incl1.setResidue(concatenatedAtomInfoVector[id1].residueID); //= {concatenatedAtomInfoVector[id1].chain, concatenatedAtomInfoVector[id1].residueID};
        IncludeAllNonBondAtomsInResidue incl2; incl2.setChain(concatenatedAtomInfoVector[id2].chain); incl2.setResidue(concatenatedAtomInfoVector[id2].residueID); //= {concatenatedAtomInfoVector[id2].chain, concatenatedAtomInfoVector[id2].residueID};
        ...
        if(itARW!= includeAllResiduesWithinVector.end() && updBiopolymerClass(incl1.getChain()).getActivePhysics())
        {
            ...
            if(it == includeAllNonBondAtomsInResidueVector.end() && updBiopolymerClass(incl2.getChain()).getActivePhysics()) {
                ...
                //cout<<__FILE__<<":"<<__LINE__<<" Just added due to dist :"<<dist<<" compared to itARW->getRadius() : "<<itARW->getRadius() <<" computed based on points:"<<concatenatedAtomInfoVector[id1].position<<", "<<concatenatedAtomInfoVector[id2].position<< endl; 
                //cout<<__FILE__<<":"<<__LINE__<<" This stretch is in the includeAllResiduesWithinVector :"<<endl; 
                //incl1.printStretch();
                //cout<<__FILE__<<":"<<__LINE__<<" This stretch is NOT in the includeAllResiduesWithinVector, but was added due to distance argument :"<<endl; 
                //incl2.printStretch();
            }
        }
...
void BiopolymerClassContainer::printAtomInfoVector() {
    map<const String, BiopolymerClass>::iterator biopolymerClassMapIterator = biopolymerClassMap.begin();
    for(biopolymerClassMapIterator = biopolymerClassMap.begin(); biopolymerClassMapIterator != biopolymerClassMap.end(); biopolymerClassMapIterator++) {
        cout<<__FILE__<<":"<<__LINE__<<" Printing atomInfoVector for chain "<<(biopolymerClassMapIterator->second).getChainID()<<endl;
...
void BiopolymerClassContainer::addIntraChainInterfaceResidues(String chain, vector<IncludeAllNonBondAtomsInResidue> & myIncludeAllNonBondAtomsInResidueVector , double radius, SimbodyMatterSubsystem & matter,State & state) {
    ...
    cout<<__FILE__<<":"<<__LINE__<<" myResidueStretchContainer.getNumResidueStretches() = "<<myResidueStretchContainer.getNumResidueStretches()<< endl;
