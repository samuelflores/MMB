...
// second arguemnt is insertion code, will default to " " if left out.
    ...
    if ( ((parameterStringClass.getString(0)).compare("nucleicAcidDuplex")      == 0) ) 
    {
     ...
     if ((parameterStringClass.getString(0)).compare("alignmentForces")      == 0) 
     {
         ...
         //MMBLOG_FILE_FUNC_LINE(ALWAYS, "alignmentForces noGap : DEPRECATED! This sets the internal parameter alignmentForcesIsGapped to False, introduces a very high penalty for gaps in the alignment, and requires that the aligned fragments have the same number of residues. If you were looking for the old \"threading\" command, set this parameter. As this is deprecated,  Now just set gapPenalty yourself, to a very negative value."<<endl);
         ...
         MMBLOG_FILE_FUNC_LINE(ALWAYS, "alignmentForces gapPenalty : This sets the gapPenalty parameter alignmentForcesGapPenalty. More negative means gaps less likely. So if you want to prevent gaps altogether, set this to a very negative value.  "<<endl);
         MMBLOG_FILE_FUNC_LINE(ALWAYS, "alignmentForces scoringScheme: This sets the SeqAn scoring scheme. Currently supported: Blosum62, Simple.   "<<endl);
          //MMBLOG_FILE_FUNC_LINE(ALWAYS, "alignmentForces gapped : This means that gappedAlignment is left at True, and Seqan is used to figure out the alignment. This is the default behavior. "<<endl); # Deprecated: insstead now just set gapPenalty to a really high value
