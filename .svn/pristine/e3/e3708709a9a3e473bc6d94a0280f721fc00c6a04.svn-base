                                                 ...
                                                 const bool readExcept)
{
    ...
    if ((parameterStringClass.getString(0)).compare("gappedThreading")      == 0) 
    {
        ...
        cout<<__FILE__<<":"<<__LINE__<<" Or    :  <gappedThreading> <Chain A>  <start residue A> <end residue A>  <Chain-B>  <start residue B> <end residue B>   [forceConstant]"<<endl;
        ...
        } else if (((parameterStringClass.getString(8)).length()==0 ) &&  ((parameterStringClass.getString(6)).length()!=0 ))   { // In case the user used the syntax  <gappedThreading> <Chain A>  <start residue A> <end residue A>  <Chain-B>  <start residue B> <end residue B>   [forceConstant]
	    String      chainA = parameterStringClass.getString(1);
	    ResidueID startResidueA = myBiopolymerClassContainer.updBiopolymerClass(chainA).residueID(userVariables, parameterStringClass.getString(2));
	    ResidueID   endResidueA = myBiopolymerClassContainer.updBiopolymerClass(chainA).residueID(userVariables, parameterStringClass.getString(3));
	    double myForceConstant = 1000000000.0; // set to super high value just to make sure it's being reset later.
	    String      chainB = parameterStringClass.getString(4);
	    ResidueID startResidueB = myBiopolymerClassContainer.updBiopolymerClass(chainB).residueID(userVariables, parameterStringClass.getString(5));
	    ResidueID   endResidueB = myBiopolymerClassContainer.updBiopolymerClass(chainB).residueID(userVariables, parameterStringClass.getString(6));
	    ...
	    cout << "READER ThreadForceConstant " << myForceConstant << endl;
	    atomSpringContainer.addGappedThreading(chainA,startResidueA, endResidueA, chainB, startResidueB, endResidueB, myForceConstant, false, myBiopolymerClassContainer);
        ...
        ((parameterStringClass.getString(0)).compare( "includeResidues") == 0) )  
    {
        ...
        for (ResidueID i = myBiopolymerClassContainer.residueID(userVariables,(parameterStringClass.getString(2)),parameterStringClass.getString(1) )  ; i <=myBiopolymerClassContainer.residueID (userVariables, (parameterStringClass.getString(3)),parameterStringClass.getString(1)); myBiopolymerClassContainer.updBiopolymerClass(parameterStringClass.getString(1)).incrementResidueID( i)  ) {
            ...
            myIncludeAllNonBondAtomsInResidue.setResidue(i);  
            myBiopolymerClassContainer.updBiopolymerClass(myIncludeAllNonBondAtomsInResidue.getChain() ).validateResidueID( myIncludeAllNonBondAtomsInResidue.getResidue() );
            physicsContainer.addStretch(myIncludeAllNonBondAtomsInResidue);  //includeAllNonBondAtomsInResidueVector.push_back(myIncludeAllNonBondAtomsInResidue);     
...
void ParameterReader::initializeFromFileOnly(const char * parameterFileName ) {
    ...
    while (inFile.good()) {
        ...
        {
            cout<<endl<<__FILE__<<":"<<__LINE__<<" read line: "<<u.str()<<endl;   
